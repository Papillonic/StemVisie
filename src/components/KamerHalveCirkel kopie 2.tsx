...
export default function KamerHalveCirkel({
  ...
  const shiftedRings = RINGS.map((ring, i) =>
    i === 0
      ? { ...ring, radius: originalRadii[0] + outerGap }
      : { ...ring, radius: originalRadii[i - 1] }
  );
    ...
    for (let segment = 0; segment < SEGMENTS; segment++) {
      for (let i = 0; i < ring.seatsPerSegment; i++) {
        ...
        const party = parties.find(
          (p) => p.abbreviation === assignment.partyAbbreviation
        );
        ...
        if (voteObj) {
          if (
            voteObj.voor >= voteObj.tegen &&
            voteObj.voor >= voteObj.onthouden
          )
            vote = "voor";
          else if (
            voteObj.tegen >= voteObj.voor &&
            voteObj.tegen >= voteObj.onthouden
          )
            vote = "tegen";
        }
  ...
  const labels = Array.from(seatsByParty.values()).map((partySeats) => {
    ...
    const radii = Array.from(
      new Set(partySeats.map((s) => s.radius))
    ).sort((a, b) => a - b);

   let labelRadius = Math.max(...radii);

// GroenLinks-PvdA één ring naar binnen
if (party.abbreviation === "GRO" && radii.length > 1) {
  labelRadius = radii[radii.length - 2];
}

const ringSeats = partySeats.filter((s) => s.radius === labelRadius);
    const avgAngle =
      ringSeats.reduce((sum, s) => sum + s.angle, 0) / ringSeats.length;

    return {
  party,
  angle: avgAngle,
  radius: labelRadius + LABEL_OFFSET,
};
  });
  ...
  return (
    <svg
      viewBox="0 0 900 600"
      className="w-full max-w-md md:max-w-3xl mx-auto scale-75 origin-top"
    >
      {seats.map((seat) => {
        ...
        return (
          <g
            key={seat.index}
            transform={`translate(${seat.x}, ${seat.y}) rotate(${rotation})`}
          >
            <polygon
              points={points}
              fill={partyColor}
      ...
      <defs>
        {labels.map((label) => {
          ...
          const angleSpan =
            (estimatedTextLength + padding) / label.radius;
      ...
      {labels.map((label) => (
        ...
        >
          <textPath
            href={`#label-path-${label.party.abbreviation}`}
            startOffset="50%"
            textAnchor="middle"
          >
            {label.party.name}
