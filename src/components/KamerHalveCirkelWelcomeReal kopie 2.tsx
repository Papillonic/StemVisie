...
interface KamerHalveCirkelProps {
  ...
  scale?: number; // optioneel, standaard 1
}
...
export default function KamerHalveCirkel({
  ...
  ringsReversed.forEach((ring) => {
    ...
    const scaleAngle = Math.min(1, usableAngle / (idealSeatAngle * seatsPerRing));
    const seatAngle = idealSeatAngle * scaleAngle;
  ...
  const labels = Array.from(seatsByParty.values()).map((partySeats) => {
    ...
    // ✅ AANPASSINGEN HIER
    let angle = avgAngle;
    let radius = outerRadius + LABEL_OFFSET;

    if (party.abbreviation === "GRO") {
      radius -= 60; // GroenLinks 1 ring naar binnen
    }

    if (party.abbreviation === "D66") {
      angle += 0.12; // D66 iets naar links
    }
  ...
  return (
    ...
    >
      ...
      {labels.map((label) => (
        <text key={label.party.abbreviation} fontSize={12} fontWeight="bold" fill="#fff">
