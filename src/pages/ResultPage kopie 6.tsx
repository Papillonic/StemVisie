...
export default function ResultPage() {
  ...
  const partyMap = Object.fromEntries(
    mockParties.map((p) => [p.abbreviation, p])
  );
  ...
  const results = useMemo(() => {
    return mockParties
      ...
      // partijen zonder stemmingen verbergen
      .filter((r) => r.totalVotes > 0)

      // sorteren op hoogste match
          ...
          {!hasVotes && (
            <div className="mb-4 p-3 rounded-lg bg-yellow-500/20 border border-yellow-400/30 text-center">
              <p className="text-yellow-200 font-bold text-sm">
                Je hebt nog geen stemmen uitgebracht — alle partijen staan op
                0%.
              </p>
            </div>
          )}
            ...
            <div className="mb-4 text-center">
              <h2 className="text-white font-bold text-xl flex flex-wrap justify-center items-center gap-2">
                ...
                <span className="text-base">
                  • {results[0].percentage}% overeenkomst op{" "}
                  {results[0].totalVotes} moties
                </span>
              </h2>
            </div>
          )}
