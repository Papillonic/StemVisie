...
export default function VotingPage() {
  ...
  // =========================
  // ✅ FIX 1: FORCE CLEAN SESSION WHEN NEW NAV DATA EXISTS
  // =========================
  const amendmentsFromNav: Amendment[] = location.state?.amendments ?? [];

  const isNewSession = amendmentsFromNav.length > 0;

  if (isNewSession) {
    localStorage.setItem("voting_amendments", JSON.stringify(amendmentsFromNav));
